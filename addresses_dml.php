...
function addresses_insert() {
	...
	insert('addresses', backtick_keys_once($data), $error);
	if($error)
	...
	if(!empty($_REQUEST['SelectedID'])) addresses_copy_children($recID, $_REQUEST['SelectedID']);

	return $recID;
}

function addresses_copy_children($destination_id, $source_id) {
	global $Translation;
...
function addresses_update($selected_id) {
	...
	sql('update `addresses` set       `address`=' . (($data['address'] !== '' && $data['address'] !== NULL) ? "'{$data['address']}'" : 'NULL') . " where `id`='".makeSafe($selected_id)."'", $o);
	if($o['error']!='') {
		echo $o['error'];
