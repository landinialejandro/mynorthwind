...
function details_insert() {
	...
	$data['vaule'] = $_REQUEST['vaule'];
		if($data['vaule'] == empty_lookup_value) { $data['vaule'] = ''; }
...
function details_update($selected_id) {
	...
	$data['item'] = makeSafe($_REQUEST['item']);
		if($data['item'] == empty_lookup_value) { $data['item'] = ''; }
	$data['vaule'] = makeSafe($_REQUEST['vaule']);
		if($data['vaule'] == empty_lookup_value) { $data['vaule'] = ''; }
	...
	sql('update `details` set       `order`=' . (($data['order'] !== '' && $data['order'] !== NULL) ? "'{$data['order']}'" : 'NULL') . ', `quantity`=' . (($data['quantity'] !== '' && $data['quantity'] !== NULL) ? "'{$data['quantity']}'" : 'NULL') . ', `item`=' . (($data['item'] !== '' && $data['item'] !== NULL) ? "'{$data['item']}'" : 'NULL') . ', `vaule`=' . (($data['vaule'] !== '' && $data['vaule'] !== NULL) ? "'{$data['vaule']}'" : 'NULL') . " where `id`='".makeSafe($selected_id)."'", $o);
	if($o['error']!='') {
		echo $o['error'];
...
function details_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0, $TemplateDV = '', $TemplateDVP = '') {
			...
			$j("#order-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_order__RAND__.value, t: 'details', f: 'order' },
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { /* */ return { s: term, p: page, t: 'details', f: 'order' }; },
	...
	if(($selected_id && !$AllowUpdate && !$AllowInsert) || (!$selected_id && !$AllowInsert)) {
		...
		$jsReadOnly .= "\tjQuery('#vaule').replaceWith('<div class=\"form-control-static\" id=\"vaule\">' + (jQuery('#vaule').val() || '') + '</div>');\n";
	...
	$lookup_fields = array('order' => array('orders', 'Order'), );
	...
	$templateCode = str_replace('<%%UPLOADFILE(vaule)%%>', '', $templateCode);
	...
	if($selected_id) {
		...
		if( $dvprint) $templateCode = str_replace('<%%VALUE(vaule)%%>', safe_html($urow['vaule']), $templateCode);
		if(!$dvprint) $templateCode = str_replace('<%%VALUE(vaule)%%>', html_attr($row['vaule']), $templateCode);
		$templateCode = str_replace('<%%URLVALUE(vaule)%%>', urlencode($urow['vaule']), $templateCode);
	}else{
		...
		$templateCode = str_replace('<%%VALUE(vaule)%%>', '', $templateCode);
		$templateCode = str_replace('<%%URLVALUE(vaule)%%>', urlencode(''), $templateCode);
	}
