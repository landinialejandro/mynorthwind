...
function contacts_insert() {
	...
	insert('contacts', backtick_keys_once($data), $error);
	if($error)
	...
	if(!empty($_REQUEST['SelectedID'])) contacts_copy_children($recID, $_REQUEST['SelectedID']);

	return $recID;
}

function contacts_copy_children($destination_id, $source_id) {
	global $Translation;
...
function contacts_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0, $TemplateDV = '', $TemplateDVP = '') {
	...
	if(is_file(dirname(__FILE__).'/hooks/contacts.type.csv')) {
		$type_data = addslashes(implode('', @file(dirname(__FILE__).'/hooks/contacts.type.csv')));
		$combo_type->ListItem = explode('||', entitiesToUTF8(convertLegacyOptions($type_data)));
		$combo_type->ListData = $combo_type->ListItem;
	}else{
		$combo_type->ListItem = explode('||', entitiesToUTF8(convertLegacyOptions("Customer;;Provider")));
		$combo_type->ListData = $combo_type->ListItem;
	}
