...
function orders_insert(){
	...
	$data['orderNumber'] = makeSafe($_REQUEST['orderNumber']);
		if($data['orderNumber'] == empty_lookup_value){ $data['orderNumber'] = ''; }
	...
	sql('insert into `orders` set       `orderNumber`=' . (($data['orderNumber'] !== '' && $data['orderNumber'] !== NULL) ? "'{$data['orderNumber']}'" : 'NULL') . ', `customer`=' . (($data['customer'] !== '' && $data['customer'] !== NULL) ? "'{$data['customer']}'" : 'NULL'), $o);
	if($o['error']!=''){
		echo $o['error'];
...
function orders_update($selected_id){
	...
	$o=array('silentErrors' => true);
	sql('update `orders` set       `orderNumber`=' . (($data['orderNumber'] !== '' && $data['orderNumber'] !== NULL) ? "'{$data['orderNumber']}'" : 'NULL') . ', `customer`=' . (($data['customer'] !== '' && $data['customer'] !== NULL) ? "'{$data['customer']}'" : 'NULL') . " where `id`='".makeSafe($selected_id)."'", $o);
	if($o['error']!=''){
		echo $o['error'];
...
function orders_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0, $TemplateDV = '', $TemplateDVP = ''){
	...
	if(($selected_id && !$AllowUpdate && !$AllowInsert) || (!$selected_id && !$AllowInsert)){
		$jsReadOnly .= "\tjQuery('#orderNumber').replaceWith('<div class=\"form-control-static\" id=\"orderNumber\">' + (jQuery('#orderNumber').val() || '') + '</div>');\n";
	...
	$templateCode = str_replace('<%%UPLOADFILE(orderNumber)%%>', '', $templateCode);
	...
	if($selected_id){
		...
		if( $dvprint) $templateCode = str_replace('<%%VALUE(orderNumber)%%>', safe_html($urow['orderNumber']), $templateCode);
		if(!$dvprint) $templateCode = str_replace('<%%VALUE(orderNumber)%%>', html_attr($row['orderNumber']), $templateCode);
		$templateCode = str_replace('<%%URLVALUE(orderNumber)%%>', urlencode($urow['orderNumber']), $templateCode);
		...
		$templateCode = str_replace('<%%VALUE(orderNumber)%%>', '', $templateCode);
		$templateCode = str_replace('<%%URLVALUE(orderNumber)%%>', urlencode(''), $templateCode);
