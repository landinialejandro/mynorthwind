...
function companies_insert() {
	...
	insert('companies', backtick_keys_once($data), $error);
	if($error)
	...
	if(!empty($_REQUEST['SelectedID'])) companies_copy_children($recID, $_REQUEST['SelectedID']);

	return $recID;
}

function companies_copy_children($destination_id, $source_id) {
	global $Translation;
...
function companies_update($selected_id) {
	...
	sql('update `companies` set       `name`=' . (($data['name'] !== '' && $data['name'] !== NULL) ? "'{$data['name']}'" : 'NULL') . ', `type`=' . (($data['type'] !== '' && $data['type'] !== NULL) ? "'{$data['type']}'" : 'NULL') . " where `id`='".makeSafe($selected_id)."'", $o);
	if($o['error']!='') {
		echo $o['error'];
...
function companies_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0, $TemplateDV = '', $TemplateDVP = '') {
			...
			$j("#type-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_type__RAND__.value, t: 'companies', f: 'type' },
						success: function(resp) {
							...
							if(resp.results[0].id == '<?php echo empty_lookup_value; ?>') { $j('.btn[id=compnayTypes_view_parent]').hide(); }else{ $j('.btn[id=compnayTypes_view_parent]').show(); }
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { /* */ return { s: term, p: page, t: 'companies', f: 'type' }; },
				...
				if(e.added.id == '<?php echo empty_lookup_value; ?>') { $j('.btn[id=compnayTypes_view_parent]').hide(); }else{ $j('.btn[id=compnayTypes_view_parent]').show(); }
	...
	$lookup_fields = array('type' => array('compnayTypes', 'Company Type'), );
